    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="w-full max-w-md p-8 space-y-6 bg-white rounded-lg shadow-md">
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            <label htmlFor="role-select" className="block text-sm font-medium text-gray-700">Login as</label>
            <select
              id="role-select"
              name="role"
              value={formData.role}
          ...
          {error && (
            <div className="text-sm text-red-600">
              {error}
              {error.includes('does not have the hall-owner role') && (
                <p>
                  Please sign in as a normal user and apply to become a hall owner.
                </p>
              )}
            </div>
          )}
